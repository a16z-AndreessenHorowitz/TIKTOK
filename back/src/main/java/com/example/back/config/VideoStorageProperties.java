...
public class VideoStorageProperties {
  ...
  public VideoStorageProperties(@Value("${app.video.storage-dir}") String storageDir) {
  ...
  private static Path resolveStorageRoot(String storageDir) {
    if (!StringUtils.hasText(storageDir)) {
      throw new IllegalArgumentException("Thiếu app.video.storage-dir trong application.properties.");
    }
